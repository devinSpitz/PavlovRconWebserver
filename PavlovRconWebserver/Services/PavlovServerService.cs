...
namespace PavlovRconWebserver.Services
{
    public class PavlovServerService
    {
        ...
        public async Task<KeyValuePair<PavlovServerViewModel, string>> CreatePavlovServer(PavlovServerViewModel server)
        {
            ...
            //Todo: Check if it works on ubuntu as well.
            //Debian has to have PAM enabled for it to work.
            string result = null;
            try
            {
                DataBaseLogger.LogToDatabaseAndResultPlusNotify("Start creting server",LogEventLevel.Verbose,_notifyService);
                //Check stuff
                ...
                DataBaseLogger.LogToDatabaseAndResultPlusNotify("Start Install pavlovserver",LogEventLevel.Verbose,_notifyService);
                
                DataBaseLogger.LogToDatabaseAndResultPlusNotify("Username used befor changed to root: "+server.SshServer.SshUsername,LogEventLevel.Verbose,_notifyService);
                result += await RconStatic.UpdateInstallPavlovServer(server,this);
                result += "\n *******************************Update/Install Done*******************************";
                
                DataBaseLogger.LogToDatabaseAndResultPlusNotify("Start Install pavlovserver service",LogEventLevel.Verbose,_notifyService);
                ...
                catch (CommandException)
                {
                    ...
                    DataBaseLogger.LogToDatabaseAndResultPlusNotify("catch after install override and remove afterwards",LogEventLevel.Verbose,_notifyService);
                    throw;
                }

                OverwrideTheNormalSSHLoginData(server, oldSSHcrid);
                DataBaseLogger.LogToDatabaseAndResultPlusNotify("Username used after override old infos: "+server.SshServer.SshUsername,LogEventLevel.Verbose,_notifyService);
                //start server and stop server to get Saved folder etc.
                
                DataBaseLogger.LogToDatabaseAndResultPlusNotify("Start after install",LogEventLevel.Verbose,_notifyService);
                try
                {
                ...
                DataBaseLogger.LogToDatabaseAndResultPlusNotify("stop after install",LogEventLevel.Verbose,_notifyService);
                try
                {
                ...
                DataBaseLogger.LogToDatabaseAndResultPlusNotify("Try to save game ini",LogEventLevel.Verbose,_notifyService);
                result +=
                ...
                DataBaseLogger.LogToDatabaseAndResultPlusNotify("write rcon file",LogEventLevel.Verbose,_notifyService);
        ...
        public virtual async Task<PavlovServer> ValidatePavlovServer(PavlovServer pavlovServer,bool root)
        {
            ...
            DataBaseLogger.LogToDatabaseAndResultPlusNotify("try to start service",LogEventLevel.Verbose,_notifyService);
            ...
            try
            {
                ...
                {
                    DataBaseLogger.LogToDatabaseAndResultPlusNotify("has to start",LogEventLevel.Verbose,_notifyService);
                    hasToStop = true;
                    ...
                    DataBaseLogger.LogToDatabaseAndResultPlusNotify("state = " + pavlovServer.ServerServiceState,LogEventLevel.Verbose,_notifyService);
                }
            }
            ...
            DataBaseLogger.LogToDatabaseAndResultPlusNotify("try to send serverinfo",LogEventLevel.Verbose,_notifyService);
            ...
            try
            {
                DataBaseLogger.LogToDatabaseAndResultPlusNotify("delete unused maps",LogEventLevel.Verbose,_notifyService);
        ...
        private async Task<PavlovServer> HasToStop(PavlovServer pavlovServer, bool hasToStop,bool root)
        {
            if (hasToStop)
            {
                DataBaseLogger.LogToDatabaseAndResultPlusNotify("stop server again!",LogEventLevel.Verbose,_notifyService);
