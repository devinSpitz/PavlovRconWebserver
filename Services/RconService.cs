    ...
    public class RconService
    {
        ...
        private async Task<ConnectionResult> SShTunnel(RconServer server,AuthType type,string command)
        {
            ...
            if (type == AuthType.PrivateKey)
            {
                ...
                connectionInfo = new ConnectionInfo(server.Adress, server.SshUsername, new PrivateKeyAuthenticationMethod(server.SshUsername,keyFiles));
            }else if (type == AuthType.UserPass)
            {
                connectionInfo = new ConnectionInfo(server.Adress, server.SshUsername,new PasswordAuthenticationMethod(server.SshUsername,server.SshPassword));
            }else if (type == AuthType.PrivateKeyPassphrase)
            {
                ...
                connectionInfo = new ConnectionInfo(server.Adress, server.SshUsername,new PasswordAuthenticationMethod(server.SshUsername,server.SshPassphrase),new PrivateKeyAuthenticationMethod(server.SshUsername,keyFiles));
            }
            ...
            using (var sftp = new SftpClient(connectionInfo))
            {
                ...
                try
                {
                    ...
                    await File.WriteAllTextAsync(commandFilelocal, server.Password+"\n"+command+"\n"+"Disconnect");
            ...
            try
            {
                sshCommandExecuteBtach.CommandTimeout = TimeSpan.FromSeconds(5);
            ...
            {
                result.errors.Add("After the ssh connection the telnet connection gives strange answers. Can not send command!");
            }
        ...
        public async Task<string> SendCommand(RconServer server, string command )
        {
            ...
            if (!connectionResult.Seccuess && server.UseSsh &&  !string.IsNullOrEmpty(server.SshKeyFileName) && File.Exists("KeyFiles/" + server.SshKeyFileName) && !string.IsNullOrEmpty(server.SshUsername))
            {
            ...
            return connectionResult.Seccuess ? connectionResult.answer : Strings.Join(connectionResult.errors.ToArray(),"\n");
        ...
        private async Task<ConnectionResult> SendCommandTelnet(RconServer server, string command)
        {
            ...
            try
            {
                ...
                using (Client client = new Client(server.Adress, server.TelnetPort,
                    new System.Threading.CancellationToken()))
                {
                    if (client.IsConnected)
                    {
                        ...
                        if (!hello.StartsWith("Password:"))
                            throw new Exception("There server " + server.Adress + " give stranges answers: " + hello);
